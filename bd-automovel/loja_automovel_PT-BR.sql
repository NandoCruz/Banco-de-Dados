...
*********************************************************************
Name: MySQL Sample Database bd_loja
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`bd_loja_automovel` /*!40100 DEFAULT CHARACTER SET latin1 */;

USE `bd_loja_automovel`;
...
CREATE TABLE `cliente` (
  ...
  `complemento_end_cliente` varchar(50) DEFAULT NULL,
  ...
  `limite_credito_cliente` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`cod_cliente`),
  KEY `cod_funcionario` (`cod_funcionario`),
  CONSTRAINT `cliente_ibfk_1` FOREIGN KEY (`cod_funcionario`) REFERENCES `funcionario` (`cod_funcionario`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `cliente` */

insert  into `cliente`(`cod_cliente`,`nome_cliente`,`ultimo_nome_cliente`,`primeiro_nome_cliente`,`telefone_cliente`,
`endereco_cliente`,`complemento_end_cliente`,`cidade_cliente`,`estado_cliente`,`cep_cliente`,`pais_cliente`,
`cod_funcionario`,`limite_credito_cliente`) values 
...
CREATE TABLE `funcionario` (
  ...
  `ultimo_nome_funcionario` varchar(50) NOT NULL,
  ...
  CONSTRAINT `funcionario_ibfk_1` FOREIGN KEY (`cod_gerente`) REFERENCES `funcionario` (`cod_funcionario`),
  CONSTRAINT `funcionario_ibfk_2` FOREIGN KEY (`cod_loja`) REFERENCES `loja` (`cod_loja`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `funcionario` */

insert  into `funcionario`(`cod_funcionario`,`ultimo_nome_funcionario`,`primeiro_nome_funcionario`,`matricula`,`email_funcionario`,
`cod_loja`,`cod_gerente`,`cargo_funcionario`) values 
...
CREATE TABLE `loja` (
  ...
  `complemento_end_loja` varchar(50) DEFAULT NULL,
  `estado_loja` varchar(50) DEFAULT NULL,
  `pais_loja` varchar(50) NOT NULL,
  `cep_loja` varchar(15) NOT NULL,
  `territorio_loja` varchar(10) NOT NULL,
  PRIMARY KEY (`cod_loja`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `offices` */

insert  into `loja`(`cod_loja`,`cidade_loja`,`telefone_loja`,`endereco_loja`,`complemento_end_loja`,`estado_loja`,
`pais_loja`,`cep_loja`,`territorio_loja`) values 
...
CREATE TABLE `pedido_detalhe` (
  ...
  `preco_cada` decimal(10,2) NOT NULL,
  `numero_fila` smallint(6) NOT NULL,
  PRIMARY KEY (`cod_pedido`,`cod_produto`),
  KEY `cod_produto` (`cod_produto`),
  CONSTRAINT `pedido_detalhe_ibfk_1` FOREIGN KEY (`cod_pedido`) REFERENCES `pedido` (`cod_pedido`),
  CONSTRAINT `pedido_detalhe_ibfk_2` FOREIGN KEY (`cod_produto`) REFERENCES `produto` (`cod_produto`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `orderdetails` */

insert  into `pedido_detalhe`(`cod_pedido`,`cod_produto`,`quantidade`,`preco_cada`,`numero_fila`) values 
...
insert  into `pedido`(`cod_pedido`,`data_pedido`,`data_entrega`,`data_envio`,`status`,`observacao`,`cod_cliente`) values 
...
CREATE TABLE `pagamento` (
  `cod_cliente` int(11) NOT NULL,
  `cod_checagem` varchar(50) NOT NULL,
  `data_pagamento` date NOT NULL,
  `valor` decimal(10,2) NOT NULL,
  PRIMARY KEY (`cod_cliente`,`cod_checagem`),
  CONSTRAINT `pagamento_ibfk_1` FOREIGN KEY (`cod_cliente`) REFERENCES `cliente` (`cod_cliente`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `payments` */

insert  into `pagamento`(`cod_cliente`,`cod_checagem`,`data_pagamento`,`valor`) values 
...
DROP TABLE IF EXISTS `produto_linha`;
...
insert  into `produto_linha`(`linha`,`descricao`,`descricao_html`,`imagem`) values 
...
CREATE TABLE `produto` (
  ...
  `qtd_estoque` smallint(6) NOT NULL,
  `preco_compra` decimal(10,2) NOT NULL,
  `preco_venda` decimal(10,2) NOT NULL,
  PRIMARY KEY (`cod_produto`),
  KEY `linha_produto` (`linha_produto`),
  CONSTRAINT `produto_ibfk_1` FOREIGN KEY (`linha_produto`) REFERENCES `produto_linha` (`linha`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `products` */

insert  into `produto`(`cod_produto`,`nome_produto`,`linha_produto`,`escala_produto`,`marca_produto`,`descricao_produto`,
`qtd_estoque`,`preco_compra`,`preco_venda`) values 
